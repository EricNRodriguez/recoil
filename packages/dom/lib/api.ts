...
export const cleanup = (node: Node): void => {
  getChildren(node).forEach(cleanup);

  getHookRegistry(node, false)?.onCleanup.forEach((h) => h());
};
...
export const isMounted = (node: Node): boolean => {
  if (
    metaNodeRegistry.has(node) &&
    metaNodeRegistry.get(node)!.vParent != null
  ) {
...
export const deregisterDelegatedEventHandler = <
...
export const setChildren = (
  ...
  if (isFrag(parent)) {
    if (parentMeta?.vParent != null) {
      setChildren(parentMeta.vParent, getChildren(parentMeta.vParent));
    }
